...
class CustomerGroupController extends GxController {
	...
	public function actionIndex() {
		...
		if (isset($_GET['CustomerGroup'])){
			$model->setAttributes($_GET['CustomerGroup']);
		}

		Yii::app()->user->setState('customer-group-grid-url', Yii::app()->request->url);
	...
	public function actionCreate() {
		...
		if (isset($_POST['CustomerGroup'])) {
			$model->setAttributes($_POST['CustomerGroup']);
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CustomerGroup');
	...
	public function actionGridviewdelete() {
		if (Yii::app()->getRequest()->getIsPostRequest()){
			...
			$criteria->compare('customer_group_id', $selectedIds);
	...
	public function actionGridviewupdate() {
		if (Yii::app()->getRequest()->getIsPostRequest()){
			...
			$criteria= new CDbCriteria;
			$criteria->compare('customer_group_id', $edittedIds);

			$models = CustomerGroup::model()->findAll($criteria);

			foreach ($models as $model){
				$model->setAttributes($editPosts[$model->customer_group_id]);
